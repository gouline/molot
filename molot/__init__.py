...
class _TargetDef:
    def __init__(
        self, name: str, description: str, group: str, phony: bool, depends: list, f
    ):
        self.name = name
...
def _list_targets_str() -> str:
    ...
    if len(targets) > 0:
        ...
        for target in targets:
            depends = (
                "(depends: {})".format(", ".join(target.depends))
                if target.depends
                else ""
            )
            ...
            print(
                "    {} - {} {}".format(target.name, target.description, depends),
                file=out,
            )
    ...
    if len(_STATE.envargs) > 0:
        ...
        for aname in _STATE.envargs:
            ...
            print(
                "  {} - {} (default: {})".format(
                    arg.name, arg.description, arg.default
                ),
                file=out,
            )
...
def envarg(
    name: str, default: str = None, description: str = "", sensitive: bool = False
) -> str:
...
def evaluate():
    ...
    parser = argparse.ArgumentParser(
        formatter_class=argparse.RawDescriptionHelpFormatter, epilog=_list_targets_str()
    )
    parser.add_argument(
        "targets",
        metavar="TARGET",
    ...
    parser.add_argument(
        "--arg", metavar="KEY=VALUE", nargs="*", help="overwrite environment arguments"
    )
    parser.add_argument(
        "--dotenv",
        metavar="PATH",
    ...
    while len(to_evaluate) > 0:
        ...
        if not deps_satisfied:
            if name in evaluated:
                print(
                    "Circular dependency detected when evaluating target {}".format(
                        name
                    )
                )
                return
